def main_model_function():
    ...
    def getlist(points, n, mode="proto"):
        ...
        pt = points[n] if mode == "proto" else points
    ...
    def getroute(query, routeRepr="summaryOnly"):
        ...
        resp = requests.get(
            f"https://atlas.microsoft.com/route/directions/json?subscription-key={subscriptionKey}&api-version=1.0&query={query}&routeType=shortest&computeBestOrder=true&travelMode=car&routeRepresentation={routeRepr}"
        ).json()
        return resp

    def cluster(points, mode):
        labels = []
        nlabels = []
        clusterpoints = []
    ...
    nearclusters = cluster(nearpoints, mode="near")
    farclusters = cluster(farpoints, mode="far")
    ...
    def maindf(paths):
        ...
        for src in range(len(paths)):
            df = pd.concat([df, srcdf_func(paths, src)], axis=0)
        df.reset_index(drop=True, inplace=True)
        return df
    ...
    # Save output to proper path
    base_path = os.path.dirname(os.path.abspath(__file__))
    fulldf.to_json(os.path.join(base_path, "fulldf.json"))
    ...
    srcdf = pd.DataFrame({
        "labels": list(range(src.shape[0])),
        "coordinates": src.T[[1, 0]].T.tolist()
    })

    destdf.to_json(os.path.join(base_path, "destdf.json"))
    srcdf.to_json(os.path.join(base_path, "srcdf.json"))
